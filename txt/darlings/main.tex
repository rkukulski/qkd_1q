...
{\color{magenta}
\textit{Proof strategy (draft).}
The argument is split into four steps. First, Proposition 1 reduces the outer
optimization to one real parameter $x>0$. Second, Lemma 2 links stationarity at
the N=1 optimum with activity of the Bob-agreement constraint. Third, Lemma 3
gives conditional smoothness of the optimal-value envelope under standard
interior-maximizer assumptions. Fourth, Lemma 4 quantifies consistency between
the candidate envelope and the converse upper bound. Combined with the
falsification harness (dense scan plus restart-best SDP), this yields a
reproducible theorem draft with explicit residuals rather than an unqualified
closed-form claim.
}
...
{\color{magenta}
\textit{Proof.}
By construction, \texttt{build\_case1\_protocol(x)} fixes the alphabet weights
$p=(1,1)$ and $q=(1)$ and uses only two preparation states:
$\ket{\psi_{1,0}}=(1,0)^T$ and
$\ket{\psi_{1,1}}=(1,x)^T/\sqrt{1+x^2}$.
All Bob and Eve quantities in the SDP are induced by these fixed tensors, so
for fixed noise level $\epsilon$ every feasible protocol in the N=1 family is
indexed by exactly one scalar $x>0$. Therefore the outer search space is
one-dimensional, equivalently parameterized by $u=\log_{10}(x)$.
}
...
{\color{magenta}
\textit{Proof.}
The feasible set is convex because it is the intersection of a PSD cone, an
affine equality, and a linear inequality. The objective is linear in $J$.
Hence any primal-dual optimum that satisfies KKT also satisfies complementary
slackness:
$\lambda_x^\star g_x(J_x^\star)=0$ with $\lambda_x^\star\ge 0$.
If $\lambda_x^\star>0$, division by $\lambda_x^\star$ gives
$g_x(J_x^\star)=0$, so the Bob-agreement inequality is active at optimum.
For the sampled positive-rate rows, the diagnostics file
\texttt{n1\_proof\_diagnostics.csv} reports positive multipliers and
near-zero primal residuals, which is a computational certificate of activity on
that grid. The boundary point $\epsilon=0$ remains numerically delicate and is
handled separately in the diagnostics summary.
}

\paragraph{Lemma 3 (envelope regularity, conditional).}
Let $R(x,\epsilon)$ denote the N=1 key-rate objective obtained after solving
the inner SDP at fixed $(x,\epsilon)$. Assume on an interval
$I\subset(0,0.25)$ that:
(i) $R$ is $C^2$ in $(x,\epsilon)$,
(ii) for each $\epsilon\in I$ there is a unique interior maximizer
$x^\star(\epsilon)$, and
(iii) $\partial^2_{xx}R(x^\star(\epsilon),\epsilon)<0$.
Then $x^\star$ is $C^1$ on $I$, and
$R^\star(\epsilon):=R(x^\star(\epsilon),\epsilon)$ is $C^1$ with
\begin{equation}
\frac{d}{d\epsilon}R^\star(\epsilon)=
\partial_\epsilon R(x^\star(\epsilon),\epsilon).
\end{equation}
{\color{magenta}
\textit{Proof.}
At an interior maximizer, first-order optimality gives
$\partial_x R(x^\star(\epsilon),\epsilon)=0$.
Assumption (iii) makes the Jacobian in $x$ non-singular, so the implicit
function theorem yields a $C^1$ map $x^\star(\epsilon)$.
Define $R^\star(\epsilon)=R(x^\star(\epsilon),\epsilon)$ and differentiate:
\[
\frac{d}{d\epsilon}R^\star(\epsilon)=
\partial_x R(x^\star,\epsilon)\frac{dx^\star}{d\epsilon}+
\partial_\epsilon R(x^\star,\epsilon).
\]
The stationarity term vanishes, leaving the envelope identity above.
}

\paragraph{Lemma 4 (converse-consistency inequality).}
Let $U(\epsilon):=\texttt{upper\_bound}(\epsilon)$,
$N(\epsilon)$ be the restart-best numerical N=1 optimum, and
$F(\epsilon):=R_{N=1}^{\mathrm{draft}}(\epsilon)$. If
$U(\epsilon)\ge N(\epsilon)$ and
$|N(\epsilon)-F(\epsilon)|\le \eta(\epsilon)$, then
\begin{equation}
F(\epsilon)\le U(\epsilon)+\eta(\epsilon),\qquad
U(\epsilon)-F(\epsilon)\ge -\eta(\epsilon).
\end{equation}
{\color{magenta}
\textit{Proof.}
From $F-N\le |F-N|\le \eta$ and $N\le U$, we get
$F\le N+\eta\le U+\eta$.
Rearranging gives $U-F\ge -\eta$.
Thus the draft formula is never more than $\eta$ above the converse curve, and
any negative gap is explicitly bounded by the observed mismatch radius.
}

{\color{magenta}
\textit{Remark (scope).}
Lemmas 2--4 turn the previous TODO items into explicit, checkable claims.
Lemma 3 is conditional on regularity hypotheses, while Lemmas 2 and 4 are
validated on the sampled grid by the tracked diagnostics artifacts.
}
