...
\maketitle
\begin{abstract}
$\epsilon, x, QKD, R$: QKD + $\epsilon$ daje $x$, QKD + $x$ daje $R$

dodaj przyklad z high qber i jak sie to ma do eps i wynikow z eps

uzasadnienie eps - jako miara do braku aborcji, tzn jezeli model jest 
spelniony 
to nie bedzie aborcji (na marginesie nikt nie powiedzial ze qber musi byc 
jedyna statystyka ktora jest sprawdzana, ale model bazowy juz moze pozostac 
jeden)

ogolnie qkd to 3 zasady: czy dziala/aborcja, czy jest bezpieczny, jakie ratio. 
dziala/aborcja mierona przez model eps, bezpieczenstwo gwarantowane przez wzor 
na destylacje bezpiecznego klucza met klasyczna oraz ratio optymalizujemy jako 
iloczyn ps i destylacji
\end{abstract}
...
For a given $\epsilon \in [0,1]$ and QKD protocol $\QQ$ we can compute secure 
key 
...
\begin{equation}
R(\QQ, E) = \PP(S)(\max_P H_2(\PP(E=P|S)) - H_2(\PP(B=A|S))).
\end{equation}
...
would like to define \textbf{the worst case} SKR $R_\epsilon(\QQ)$ 
within the 
noise model with the intensity $\epsilon$. The value of $\PP(B=A|S)$ 
can be optimized within the scope of the model we assumed. That value gives us 
threshold for which we can optimize further $\PP(S)$ and Eve knowledge 
$\PP(E=P|S)$ to guarantee unconditional security.

We define:
\begin{itemize}
\item $\PP_\epsilon(B=A|S) = \max\left\{\min_{\Phi \sim \epsilon} 
\PP(B=A|S), \frac12 \right\}$,
\item $\PP_\epsilon(S) = \min_{\Phi}\{ \PP(S): \PP(B=A|S) \ge 
\PP_\epsilon(B=A|S) \}$,
...
\subsection{Goal}
For a given $\epsilon \in [0,1]$ and the QKD protocol $\QQ$ we define the worst 
case 
SKR as
\begin{equation}
R_\epsilon(\QQ) = \PP_\epsilon(S)(H_2(\min_P \PP_\epsilon(E=P|S)) - 
H_2(\PP_\epsilon(B=A|S))).
\end{equation}

Our goal is to find (numerically / analytically)
\begin{equation}
R^*_\epsilon = \sup_{\QQ \in \text{QKD}}R_\epsilon(\QQ).
\end{equation}
...
$\epsilon \in [0,1]$
\subsection{Output:}
A function that computes $\epsilon \mapsto \sup_{\QQ \in \text{QKD}} 
R_\epsilon(\QQ)$, where 
$R_\epsilon(\QQ)$ is the minimal SKR for given $\epsilon$ and QKD $\QQ$.
...
\otimes \proj{\phi_{i,b}}$ and $W_S = \sum_{i,a}  
p_{i,a} \overline{\proj{\psi_{i,a}}} 
...
Let $x = \PP_\epsilon(B=A|S)$. We have
\begin{equation}
\begin{split}
\PP_\epsilon(S) &= \min_{\Phi}\{ \PP(S): \PP(B=A|S) \ge 
x \}\\
=&\min_{J_\Phi} \left(\tr\left(J_\Phi 
Q_S\right):  \frac{\tr\left(J_\Phi W_S\right)}{\tr\left(J_\Phi 
Q_S\right)} \ge x, J_\Phi \ge 0, \tr_2(J_\Phi) 
= \Id_2 \right)\\
=&\min_{J_\Phi} \left(\tr\left(J_\Phi 
Q_S\right):  \tr\left(J_\Phi W_S\right)\ge x\tr\left(J_\Phi 
Q_S\right), J_\Phi \ge 0, \tr_2(J_\Phi) 
= \Id_2 \right).
\end{split}
\end{equation}
