...
We performed a numerical optimization of the worst-case Secure Key Rate
($R_\epsilon$) for varying noise levels $\epsilon$ and protocol sizes
$N \in \{1, 2, 3, 4\}$. The optimization was conducted using an Artificial Bee
Colony (ABC) heuristic, which proved effective in finding optimal protocol
configurations that maximize $R_\epsilon$ under the strict security
constraints.

\subsection{Performance Analysis}
The results, illustrated in Figure~\ref{fig:results}, demonstrate a clear
trade-off between the maximum achievable rate at low noise and the robustness
of the protocol at high noise.
\begin{itemize}
    \item \textbf{High Peak Rate (N=1)}: For low noise ($\epsilon \approx 0$),
    simpler protocols with $N=1$ achieve the highest Secure Key Rate
    ($R \approx 0.69$). However, their performance degrades rapidly as noise
    increases.
    \item \textbf{Robustness (N=4)}: Increasing the number of pairs $N$
    significantly improves robustness. For $N=4$, the Secure Key Rate remains
    remarkably stable, maintaining $R > 0.20$ even at $\epsilon=0.01$
    (1\% channel noise). This suggests that higher-dimensional protocols are
    preferable in noisy environments.
\end{itemize}

\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.8\textwidth]{../results_sweep_20260114_160215.png}
    \caption{Worst-case Secure Key Rate ($R_\epsilon$) as a function of noise
    intensity $\epsilon$ for different numbers of pairs $N$. Protocols with
    higher $N$ exhibit greater stability against noise.}
    \label{fig:results}
\end{figure}

\subsection{Example of a Robust Protocol}
We explicitly present the parameters of the best protocol found for $N=4$ at
$\epsilon=0.01$, which achieves $R \approx 0.2077$.

\textbf{Probabilities ($p_{i,a}$):}
\begin{align*}
p_{1,0} &= 0.512, & p_{1,1} &= 0.488 \\
p_{2,0} &= 0.469, & p_{2,1} &= 0.531 \\
p_{3,0} &= 0.794, & p_{3,1} &= 0.206 \\
p_{4,0} &= 0.520, & p_{4,1} &= 0.480
\end{align*}

\textbf{States ($\ket{\psi_{i,a}}$):}
(Approximate values)
\begin{itemize}
    \item Pair 1: $\ket{\psi_{1,0}} \approx \begin{pmatrix} 0.967\\ -0.019 \end{pmatrix},
    \quad \ket{\psi_{1,1}} \approx \begin{pmatrix} 0.206\\ -0.689 \end{pmatrix}$
    \item Pair 2: $\ket{\psi_{2,0}} \approx \begin{pmatrix} 0.751\\ -0.564 \end{pmatrix},
    \quad \ket{\psi_{2,1}} \approx \begin{pmatrix} 0.542\\ 0.696 \end{pmatrix}$
    \item Pair 3: $\ket{\psi_{3,0}} \approx \begin{pmatrix} 0.672\\ -0.335 \end{pmatrix},
    \quad \ket{\psi_{3,1}} \approx \begin{pmatrix} 0.686\\ -0.220 \end{pmatrix}$
    \item Pair 4: $\ket{\psi_{4,0}} \approx \begin{pmatrix} 0.684\\ -0.145 \end{pmatrix},
    \quad \ket{\psi_{4,1}} \approx \begin{pmatrix} 0.548\\ -0.520 \end{pmatrix}$
\end{itemize}

\textbf{Parameters ($q_i$):}
\[ q = [0.8140, 0.0815, 0.0419, 0.7448] \]

\subsection{High-Rate Protocol Example (N=1)}
For comparison, we present a protocol optimized for low noise
($\epsilon \approx 2.5 \times 10^{-5}$), which achieves a high Secure Key Rate
of $R \approx 0.775$. This protocol uses only $N=1$ pair of states.

\textbf{Probabilities:}
\[ p_{1,0} \approx 0.500, \quad p_{1,1} \approx 0.500 \]

\textbf{States:}
Aligning the first state with the standard basis vector $\ket{0}$:
\begin{align*}
\ket{\psi_{1,0}} &\approx \ket{0} \\
\ket{\psi_{1,1}} &\approx \begin{pmatrix} 0.072 - 0.093i \\ 0.184 + 0.976i \end{pmatrix}
\end{align*}
The optimal states for this low-noise regime are nearly orthogonal, enabling
high capacity while satisfying the security constraints through the
parameter $q$.

\textbf{Parameter:}
\[ q_1 \approx 0.089 \]

\subsection{Intermediate Protocols (N=2 and N=3)}
We also investigated intermediate protocol sizes. For $N=2$ at $\epsilon=0.01$,
we found a protocol with $R \approx 0.427$. Interestingly, the first pair of
states in this protocol is nearly orthogonal, while the second pair provides
additional structure to handle noise.

\textbf{N=2 Example Parameters ($p_{i,a}$):}
\begin{align*}
p_{1,0} &= 0.508, & p_{1,1} &= 0.492 \\
p_{2,0} &= 0.540, & p_{2,1} &= 0.460
\end{align*}

\textbf{N=2 States ($\ket{\psi_{i,a}}$):}
\begin{itemize}
    \item Pair 1: $\ket{\psi_{1,0}} \approx \begin{pmatrix} 0.808\\ 0.368 \end{pmatrix},
    \quad \ket{\psi_{1,1}} \approx \begin{pmatrix} 0.382\\ -0.814 \end{pmatrix}$
    \item Pair 2: $\ket{\psi_{2,0}} \approx \begin{pmatrix} 0.053\\ 0.423 \end{pmatrix},
    \quad \ket{\psi_{2,1}} \approx \begin{pmatrix} 0.762\\ -0.537 \end{pmatrix}$
\end{itemize}

For $N=3$, we observed similar mixing strategies, where some pairs resemble
standard orthogonal bases while others are tilted to provide noise immunity.

\textbf{Disclaimer:} The numerical implementation, optimization results, and 
the text of this section were generated with the assistance of an AI system.
