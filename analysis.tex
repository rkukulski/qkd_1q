...
$\Phi=\Id$ but the more the noise is intensive or Eve attacks harder then 
$\Phi$ drift away from $\Id$. To measure the intensity of the noise, we will 
...
Right now, we would like to define \textbf{the worst case} SKR $R_\epsilon$ 
within the 
...
\item $\PP_\epsilon(B=A|S) = \min_{\Phi \sim \epsilon} \PP(B=A|S)$.
...
$\ket{\psi_{i,a}}$ satisfy $\|\psi_{i,a}\|_2=1$ and pairs of vectors 
...
and \begin{equation}
\sum_{i,a} 
\frac{q_i}{p_{i,a}} \proj{\psi_{i,a \oplus 1}^\perp} \le \Id_2.\end{equation}
\end{itemize}
\subsection{Input:}
$\epsilon \in [0,1]$
\subsection{Output:}
...
We start by putting important assumption that reduces the number of parameters 
which define the given QKD protocol. If $\epsilon=0$ then the Secure key 
agreement has to work at its best, which means parties' raw keys are perfectly 
correlated. Hence, if Alice 
...
Moreover, there is no leakage of information to Eve. That means for each $i$ 
there can not be any bias between $0$ and $1$. That indicate $\ket{\phi_{i,b}} 
= \sqrt{\frac{q_i}{p_{i,b}}}\ket{\psi_{i,b\oplus 1}^\perp}$ for some $q_i > 0$.
\subsection{$\PP_\epsilon(S)$:}
Let $Q_S = \sum_{i,a,b} \frac{q_i 
p_{i,a}}{p_{i,b}}(\overline{\proj{\psi_{i,a}}} 
\otimes \proj{\psi_{i,b\oplus 1}^\perp})$.
\begin{equation}
\begin{split}
...
Let $W_S = \sum_{i,a} q_i(\overline{\proj{\psi_{i,a}}} 
\otimes \proj{\psi_{i,a\oplus 1}^\perp})$.
\begin{equation}
\begin{split}
